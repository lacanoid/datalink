...
Currently, the file filter is available as:
- mod_perl handler for Apache2 web server. This can be used with URLs containing read access tokens,
such as those generated by `DLURLCOMPLETE()` function.
- special `dlcat` shell command to read files which behaves like UNIX command `cat`, 
but for filenames containing read access tokens,
such as those generated by `DLURLPATH()` function.
- SQL [file functions](https://github.com/lacanoid/datalink/blob/master/docs/functions.md), 
such as `read_text` and `write_text`.

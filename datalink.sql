...
--
--  datalink
--  version 0.26.0210 lacanoid@ljudmila.org
--
--------------------------------------------------------------- ---------------
...
CREATE OR REPLACE FUNCTION curl_perform(
...
INSERT INTO http_response_codes VALUES 
 ...
 (509, 'Bandwidth Limit Exceeded', 'Apache HTTP Server'),
...
AS $_$ 
   select case 
          ...
          then datalink.dl_url_makeinsight(pg_catalog.dlurlcompleteonly($1)
...
AS $_$ select datalink.uri_get(
  case when datalink.is_local($1)
       then coalesce((
               select dirurl||substr(pg_catalog.dlurlpathonly($1),length(dirpath)+1)
                 from datalink.directory
...
CREATE OR REPLACE FUNCTION has_web_privilege(
  url text, privilege text default 'SELECT', allowsuper boolean default true) 
RETURNS boolean LANGUAGE sql AS 
$$select datalink.has_web_privilege(current_role::regrole,$1,$2,$3)$$;
...
BEGIN
  ...
  m := regexp_matches(mypath,'^(.*/)(([a-z0-9\-]{36})[,;_:@])?(.*)$','i');
  mypath := coalesce(m[1]||m[4],mypath);
  t := m[3]::datalink.dl_token;
  mypath := regexp_replace(mypath,'^/dlfs/','/');
  if t is not null then
   update datalink.dl_new_files
      set op='t'
   ...
   using errcode = 'HW004', 
          detail = 'file corresponding to a write token was not found',
            hint = 'write tokens are embedded in filenames generated by DLURLPATHWRITE() function';
  end if;
  return null; -- not a filename with write token
END
$$;
